...
function editorUrl(segment, baseDir) {
  return `/front/corrector/editor?segment=${encodeURIComponent(segment)}&base_dir=${encodeURIComponent(baseDir || "")}&output_dir=${encodeURIComponent(EXPORT_DIR)}`;
}
...
async function loadDetails(segment, baseDir, outputDir) {
  detailsBox.innerHTML = "Chargement des details...";

  try {
    const outputQuery = outputDir ? `&output_dir=${encodeURIComponent(outputDir)}` : "";
    const framesRes = await fetch(`/correct/frames?segment=${encodeURIComponent(segment)}&base_dir=${encodeURIComponent(baseDir)}${outputQuery}`);
    const trajRes = await fetch(`/correct/trajectories?segment=${encodeURIComponent(segment)}&base_dir=${encodeURIComponent(baseDir)}${outputQuery}`);
    ...
    detailsBox.innerHTML = `
      ...
      <p><strong>Output dir:</strong> ${outputDir || EXPORT_DIR}</p>
...
segmentsList.addEventListener("click", (event) => {
  ...
  loadDetails(segment, baseDir, outputDir);
});
